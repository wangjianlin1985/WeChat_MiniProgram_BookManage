...
Page({
  ...
  // 加载读者类型列表
  listReaderType: function () {
    var self = this
    var url = config.basePath + "api/readerType/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          readerTypes: self.data.readerTypes.concat(res.data.list),
  ...
  //删除读者类型记录
  removeReaderType: function (e) {
    var self = this
    var readerTypeId = e.currentTarget.dataset.readertypeid
    wx.showModal({
      title: '提示',
      content: '确定要删除readerTypeId=' + readerTypeId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/readerType/delete/" + readerTypeId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除读者类型后客户端同步删除数据
            var readerTypes = self.data.readerTypes;
            for (var i = 0; i < readerTypes.length; i++) {
              if (readerTypes[i].readerTypeId == readerTypeId) {
                readerTypes.splice(i, 1)
                break
              }
            }
            self.setData({ readerTypes: readerTypes })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
